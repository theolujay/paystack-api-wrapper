...
def test_validate_amount_and_email_non_numeric_amount():
    with pytest.raises(
        ValidationError,
        match="Amount must be a valid number string without comma or decimal",
    ):
...
def test_validate_charge_authorization_missing_authorization_code():
    with pytest.raises(APIError, match="Authorization code is required"):
        _validate_charge_authorization(
            email="test@example.com", amount=10000, authorization_code=""
        )
...
def test_validate_charge_authorization_valid():
    # Should not raise any exception
    _validate_charge_authorization(
        email="test@example.com", amount=10000, authorization_code="AUTH_testcode"
    )
